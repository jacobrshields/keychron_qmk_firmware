// Based off keyboards/keychron/v3_max/ansi_encoder/keymaps/via/keymap.c
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_tkl_ansi(
        KC_ESC,   KC_BRID,  KC_BRIU,  KC_MCTRL, KC_LNPAD, RGB_VAD,  RGB_VAI,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,  KC_VOLU,    KC_MUTE,    KC_SNAP,  KC_SCRL,  XXXXXXX,
...
bool rgb_matrix_indicators_user(void) {
    #if defined(ENABLE_TOGGLEABLE_BATTERY_INDICATOR) && defined(RGB_MATRIX_ENABLE)
        if (!rgb_matrix_indicators_toggleable_battery_indicator()) {
            return false; // Stop processing
        }
    #endif
    return true; // Continue processing
